...
def _run_pdf_extract(
    *,
    ...
    def emit(msg: str) -> None:
        if on_progress and msg.strip():
            on_progress(msg.rstrip())
    ...
    if returncode != 0:
        ...
        if "image input" in tail_lower or "vision" in tail_lower and "support" in tail_lower:
            raise LLMError(
                "PDF 转 Markdown 需要支持图像输入的视觉模型。"
                "请在「设置 → 文档提取模型」中更换为 vision 模型（当前模型不支持 image input）。"
                f" 详情: {tail[-400:]}"
            )
        raise LLMError(f"PDF 提取失败 ({returncode}): {tail}")
