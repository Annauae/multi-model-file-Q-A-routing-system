...
DEBUG_RESTART_EXIT_CODE = 42


def _schedule_server_restart(*, reason: str, delay_s: float = 1.0) -> None:
    """调试超时等场景：延迟后退出 worker，由 uvicorn --reload 或启动脚本拉起新进程。"""
    def _worker() -> None:
        time.sleep(delay_s)
        os._exit(DEBUG_RESTART_EXIT_CODE)
    ...
    *,
    ...
    def _check_cancel() -> None:
        if cancel_event is not None and cancel_event.is_set():
            raise LLMError(f"请求超时（{settings.debug_request_timeout_s}s）")
...
def create_app() -> FastAPI:
    ...
    def ask_confidence_stream(req: ConfidenceAskRequest) -> StreamingResponse:
        ...
        def gen() -> Iterator[str]:
            ...
            timeout_s = settings.debug_request_timeout_s

            while True:
                if time.perf_counter() - started >= timeout_s and "payload" not in result_box and not error_box:
                    cancel_event.set()
                    detail = f"请求超时（{timeout_s}s）"
                    yield _sse("error", {"detail": detail, "timed_out": True})
                    return
                try:
            ...
            if error_box:
                err = error_box[0]
                timed_out = cancel_event.is_set() or "超时" in str(err)
                yield _sse("error", {"detail": str(err), "timed_out": timed_out})
                return
