...
def _raise_friendly_llm_error(e: Exception) -> None:
    ...
    if "AuthenticationError" in msg or "401" in msg or "API key format is incorrect" in msg:
        raise LLMError(
            "模型鉴权失败（401）。请检查「设置」页中对应模型的 API Key 是否正确，修改后需刷新页面或重启服务。"
        ) from e
...
class LLMClient:
    ...
    def __init__(self, settings: Settings, *, api_base_url: str | None = None, api_key: str | None = None, enable_thinking: bool | None = None):
        ...
        if not self._settings.mock_llm and not self._api_key and not self._is_ollama():
            raise LLMError("未配置 API Key，无法调用模型。请在「设置」页填写 API Key，或选择 Ollama 本地模型（无需 Key）。")
    ...
    def _ollama_think_enabled(self) -> bool | None:
        return self._effective_enable_thinking()
    ...
    def _build_extra_body(self, *, model: str) -> Dict[str, Any]:
        extra: Dict[str, Any] = {}
        enable_thinking = self._effective_enable_thinking()
        if self._is_volc_ark() and enable_thinking is not None:
            extra["thinking"] = {
        ...
        elif enable_thinking is not None and self._model_supports_enable_thinking(model):
