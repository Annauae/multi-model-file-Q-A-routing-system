...
function renderLogEntry(entry) {
  ...
  if (detail.length > maxLen) {
    detail = `${detail.slice(0, maxLen)}…（已截断）`;
  }
  return `<div class="logBlock ${escapeHtml(kind)}"><span class="logLine">${escapeHtml(ts)} ${escapeHtml(mod)}${escapeHtml(kb)} ${escapeHtml(detail)}</span></div>`;
}
...
async function fetchLogs(module) {
  ...
  try {
    ...
    box.innerHTML = `<div class="empty">加载失败：${escapeHtml(e.message || String(e))}</div>`;
  }
}
...
function bindLogsNav() {
  ...
  $("#logsSubNav")?.addEventListener("click", (e) => {
    ...
    fetchLogs(btn.dataset.logModule || "");
  });
}
...
async function logsViewEnter() {
  ...
  fetchLogs(logsCurrentModule);
  if (logsPollTimer) clearInterval(logsPollTimer);
  logsPollTimer = setInterval(() => {
    if (!logsPaused && currentModule === "logs") fetchLogs(logsCurrentModule);
  }, 3000);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#logsKbSelect")?.addEventListener("change", () => fetchLogs());
});
