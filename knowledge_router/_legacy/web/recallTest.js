...
function renderRecallTimeoutMetrics() {
  const html = `<div class="metricTimeout">超时</div>`;
  const tp = $("#recallTimingPanel");
  const tok = $("#recallTokenPanel");
...
function renderRecallMetrics(rows) {
  ...
  renderTimingsPanel($("#recallTimingPanel"), totals, "运行后显示", "ask");
  renderTokenPanel(
    $("#recallTokenPanel"),
    { tokens: tokenSum, token_breakdown: breakdown },
    "运行后显示"
  );
  if (typeof switchRightTab === "function") switchRightTab("timing");
}

async function batchRunRecall(btn) {
  let all;
  ...
  const runBtn = btn || $("#recallRunBtn");
  await withButtonRunning(runBtn, "运行中…", async () => {
    const ran = [];
    try {
      ...
      showToast(`已运行 ${ran.length} 条`);
      renderRecallMetrics(ran);
    } catch (e) {
      if (isAskTimeoutError(e)) {
        renderRecallTimeoutMetrics();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $$("#recallActionsDropdown [data-recall-action]").forEach((item) => {
    item.addEventListener("click", () => {
      closeAllDropdowns();
      const action = item.dataset.recallAction;
      if (action === "addRow") {
        ...
        saveRecallTests().catch((e) => showToast(e.message, "error"));
        ...
        recallExportMarkdown();
      }
    });
  });

  $("#recallRunBtn")?.addEventListener("click", () => batchRunRecall($("#recallRunBtn")));
