...
function renderImportMetrics(data, targets) {
  importLastMetrics = data || null;
  const t = targets || {};
  const timingEl = t.timingEl || $("#extractModalTimingPanel");
  const tokenEl = t.tokenEl || $("#extractModalTokenPanel");
  renderTimingsPanel(timingEl, data?.timings, "运行后显示", "import");
  renderTokenPanel(
    tokenEl,
...
function restoreImportMetrics(targets) {
  if (importLastMetrics) renderImportMetrics(importLastMetrics, targets);
  else clearImportMetrics(targets);
}

function clearImportMetrics(targets) {
  importLastMetrics = null;
  const t = targets || {};
  const timingEl = t.timingEl || $("#extractModalTimingPanel");
  const tokenEl = t.tokenEl || $("#extractModalTokenPanel");
  renderTimingsPanel(timingEl, null, "运行后显示", "import");
  renderTokenPanel(tokenEl, null, "运行后显示");
}

const EXTRACT_METRICS_TARGET = {
  timingEl: () => $("#extractModalTimingPanel"),
  tokenEl: () => $("#extractModalTokenPanel"),
};

const GENERATE_METRICS_TARGET = {
  timingEl: () => $("#generateModalTimingPanel"),
  tokenEl: () => $("#generateModalTokenPanel"),
};

function extractMetricsTarget() {
  return { timingEl: EXTRACT_METRICS_TARGET.timingEl(), tokenEl: EXTRACT_METRICS_TARGET.tokenEl() };
}

function generateMetricsTarget() {
  return { timingEl: GENERATE_METRICS_TARGET.timingEl(), tokenEl: GENERATE_METRICS_TARGET.tokenEl() };
}
