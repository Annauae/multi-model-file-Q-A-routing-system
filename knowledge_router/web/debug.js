...
async function askConfidenceStream() {
  ...
  await withButtonRunning(btn, "运行中…", async () => {
    ...
    renderTimingsPanel($("#debugTimingPanel"), null);
    renderTokenPanel($("#debugTokenPanel"), null);
    ...
    try {
      ...
      await consumeSseStream(resp, (evt) => {
        ...
        if (evt.event === "done") {
          ...
          renderTimingsPanel($("#debugTimingPanel"), d.timings);
          renderTokenPanel($("#debugTokenPanel"), d.timings);
        }
        if (evt.event === "error") showToast(evt.data.detail || "错误", "error", 3200);
      });
    } catch (e) {
