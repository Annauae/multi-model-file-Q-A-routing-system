...
function keyInputHtml(className, apiKey) {
  ...
  return `<span class="keyInputWrap">
        <input class="settingsInput ${className} keyInput" type="text" data-field="api_key" data-real-key="${escapeHtml(key)}" value="${escapeHtml(maskedVal)}" placeholder="可留空" autocomplete="off" readonly />
...
function renderSlotForm(key, cfg) {
  ...
  return `<div class="slotCard" data-slot="${escapeHtml(key)}">
    <label class="fieldLabel">API Base URL<input class="settingsInput slot-api" data-field="api_base_url" value="${escapeHtml(cfg?.api_base_url || "")}" /></label>
    <label class="fieldLabel">API Key ${keyInputHtml("slot-api", cfg?.api_key)}</label>
    <label class="fieldLabel">Model<input class="settingsInput slot-api" data-field="model" value="${escapeHtml(cfg?.model || "")}" /></label>
...
function renderProfileCard(p, isDefault) {
  return `<div class="profileCard" data-profile-id="${escapeHtml(p.id)}">
    <div class="profileCardHead">
      <label class="fieldLabel profileNameField">模型名称<input class="settingsInput profileName profile-field" data-field="name" value="${escapeHtml(p.name || p.id)}" placeholder="例如：GPT-4o 回答" /></label>
    ...
    <label class="fieldLabel">API Base URL<input class="settingsInput profile-field" data-field="api_base_url" value="${escapeHtml(p.api_base_url || "")}" /></label>
    <label class="fieldLabel">API Key ${keyInputHtml("profile-field", p.api_key)}</label>
    <label class="fieldLabel">Model<input class="settingsInput profile-field" data-field="model" value="${escapeHtml(p.model || "")}" /></label>
